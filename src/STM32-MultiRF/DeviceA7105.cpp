...
u8 DeviceA7105::writeData(const u8 *data, u8 length, u8 channel)
{
    ...
    A7105_CS_LO();
    mSPI->transfer(A7105_RST_WRPTR);
    u8 res = mSPI->transfer(0x05);
    for (i = 0; i < length; i++)
        mSPI->transfer(*data++);
    A7105_CS_HI();
...
void DeviceA7105::writeID(u32 id)
{
    A7105_CS_LO();
    mSPI->transfer(0x06);
    mSPI->transfer((id >> 24) & 0xFF);
    mSPI->transfer((id >> 16) & 0xFF);
    mSPI->transfer((id >> 8) & 0xFF);
    mSPI->transfer((id >> 0) & 0xFF);
    A7105_CS_HI();
}