...
u8 DeviceCYRF6936::writeRegMulti_P(u8 reg, const u8 *data, u8 length)
{
    ...
    u8 res = mSPI->transfer(0x80 | reg);
    for (u8 i = 0; i < length; i++) {
        mSPI->transfer(pgm_read_byte(data + i));
    }
    CYRF_CS_HI();
    return res;
}
...
{
    CYRF_CS_LO();
    mSPI->transfer(0x80 | 0x24);
    mSPI->transfer(preamble & 0xff);
    mSPI->transfer((preamble >> 8) & 0xff);
    mSPI->transfer((preamble >> 16) & 0xff);
    CYRF_CS_HI();
}
